...
- **核心交互**：靠近目标并按 `E`
- **构建入口场景**：`Assets/Scenes/farm.unity`
...
`farm.unity` 中可放置一个手动搭建的首屏 UI，例如 `Canvas > StartPanel > BackgroundImage/Button/Text`。背景图使用 `Assets/Textures/UI/background.png`，开始按钮通过 `Button.onClick` 绑定 `PressAnyKeyStartUI.EnterGame()`，点击后隐藏首屏面板并恢复游戏时间。`MainMenu.unity` 仍保留为独立主菜单场景，可按需要继续使用。
...
Assets/
  ...
  Scenes/              Unity 场景
    farm.unity         当前构建入口和主要农场场景
    MainMenu.unity     独立主菜单场景
    UI.unity           UI 测试/组合场景
    home.unity         房屋/室内相关场景
    Pasture.unity      牧场场景
    farm_B_test.unity  农业模块测试场景
    A_Rest_Test.unity  休息/跨天测试场景
  Scripts/
    Audio/             音频管理
...
- `Assets/Scripts/UI/PressAnyKeyStartUI.cs`：首屏 UI 开始按钮逻辑，隐藏开始面板并恢复游戏时间。
- `Assets/Scripts/UI/MainMenu.cs`：独立主菜单按钮逻辑，可加载指定场景。
